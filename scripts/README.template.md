...
2. **Search agents**: Search is becoming a trajectory-control problem rather than a query-rewrite module. CAFE and CAS optimize when to seek feedback and how complete trajectories enter RL, while ReTree and LoongReflect make reflection a reversible memory-control action. ITER trains retrieval on prior interaction state, Beyond One-Shot Expansion explicitly targets unresolved evidence over multiple rounds, and RAP uses temporal access controls plus frozen-evidence replay to expose acquisition bias. NIS-Agent, Marco DeepResearch, SciLENS, and HAE-GEO increasingly treat evidence isolation, verification, and recovery from poisoned sources as first-class trajectory events. Agentic-R, Headroom-Drift, and SignBalance then optimize the policy–retriever–replay layer that produces those trajectories.
...
Planning · Memory · Tool Use · RL are converging in 2026: AgenticRec, Agentic-R, and ITER connect tool/retrieval trajectories to outcome-sensitive learning; MARS, CoVeMem, AtomRec, and HyperTrace turn memory into managed, trainable, or hypothesis-preserving state; Marco, SciLENS, Beyond One-Shot Expansion, and HAE-GEO couple evidence navigation with verification and recovery; CORAL, DMRL, IGPO, AutoLR, Astar, and A/B Agent extend the same loop to live industrial configuration and strategy iteration.
